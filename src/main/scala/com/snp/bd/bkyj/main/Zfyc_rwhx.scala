...
/*    val userData = Array("20180103112945,18874021293,wt026vza",
    ...
    val gjRdd = cdr.rdd.map(row => (row.getString(0) + "@" + row.getString(1).substring(6, 8), Array((row.getString(1), row.getString(2))))).reduceByKey(_ ++ _).flatMap(v => {
        ...
        if (!s._2.equals(last)) {
          if (lasths.length > 0) {
            ...
            if (subt > h) {
              h = subt
              maxgeo = last
              if (s._1.substring(8, 10).toInt <= 6) {
                maxgeo0_4 = maxgeo
              } else if (s._1.substring(8, 10).toInt <= 23) {
                maxgeo6_18 = maxgeo
              }
            }
          }
      ...
      listBf.append((v._1.split("@")(0)+"@"+maxgeo0_4+"@"+maxgeo6_18,1))
      listBf.iterator
    }).reduceByKey(_+_).filter(v=>v._2>20).foreachPartition(iterator => {
      ...
      for (row <- iterator) {
        ...
        if((!row._1.split("@")(1).equals(czd))&&row._1.split("@")(2).equals(czd)){
          ...
          put.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes("zfyc"), Bytes.toBytes("1"))
          putList.add(put)
        }
      }
      if (putList.size() > 0) {
